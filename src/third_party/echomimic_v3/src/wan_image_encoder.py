    ...
    def from_pretrained(cls, pretrained_model_path, transformer_additional_kwargs={}):
        ...
        if not os.path.exists(pretrained_model_path):
            raise FileNotFoundError(
                f"Model file not found: {pretrained_model_path}\n"
                f"Please ensure the model file exists at this path."
            )
        
        # Check file size (corrupted files are often very small)
        file_size = os.path.getsize(pretrained_model_path)
        if file_size < 1024:  # Less than 1KB is suspicious
            raise ValueError(
                f"Model file appears to be corrupted or incomplete: {pretrained_model_path}\n"
                f"File size is only {file_size} bytes, which is too small for a valid model file.\n"
                f"Please re-download the model file."
            )
        ...
        try:
            ...
            if "failed finding central directory" in str(e) or "zip archive" in str(e).lower():
                raise RuntimeError(
                    f"Failed to load model file: {pretrained_model_path}\n"
                    f"The file appears to be corrupted or incomplete (PyTorch zip archive error).\n"
                    f"File size: {file_size} bytes\n"
                    f"Please re-download the model file. The download may have been interrupted.\n"
