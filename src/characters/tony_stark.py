...
class TonyStark(CharacterBase):
    """
    Character class for Tony Stark.
    
    This class extends CharacterBase with specific configuration for Tony Stark,
    a brilliant inventor, engineer, and tech entrepreneur.
    """
    ...
    def __init__(
        ...
        """
        Initialize Tony Stark character.
        
        Args:
            image_paths: List of paths to Tony Stark images
            voice_samples: List of paths to Tony Stark voice sample audio files
            dataset_path: Path to the Tony Stark dataset folder
            persona_llm_ckpt_path: Path to the Tony Stark persona LLM checkpoint
            video_paths: List of paths to Tony Stark videos
            yaml_path: Path to the tony_stark.yaml config file (defaults to characters/tony_stark/tony_stark.yaml)
        """
        super().__init__(
        ...
        if yaml_path is None:
            # Default to characters/tony_stark/tony_stark.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "tony_stark" / "tony_stark.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "tony_stark" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Tony Stark")
    
    def __repr__(self) -> str:
        """String representation of the Tony Stark character."""
        return (
            f"TonyStark("
            f"name={self.name}, "
