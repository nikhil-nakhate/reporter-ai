...
class YannLeCun(CharacterBase):
    """
    Character class for Yann LeCun.
    
    This class extends CharacterBase with specific configuration for Yann LeCun,
    a world-renowned AI researcher and computer scientist.
    """
    ...
    def __init__(
        ...
        """
        Initialize Yann LeCun character.
        
        Args:
            image_paths: List of paths to Yann LeCun images
            voice_samples: List of paths to Yann LeCun voice sample audio files
            dataset_path: Path to the Yann LeCun dataset folder
            persona_llm_ckpt_path: Path to the Yann LeCun persona LLM checkpoint
            video_paths: List of paths to Yann LeCun videos
            yaml_path: Path to the yann.yaml config file (defaults to characters/yann/yann.yaml)
        """
        super().__init__(
        ...
        self.name = "Yann LeCun"
        
        # Load YAML configuration
        if yaml_path is None:
            # Default to characters/yann/yann.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "yann" / "yann.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "yann" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Yann LeCun")
    
    def __repr__(self) -> str:
        """String representation of the Yann LeCun character."""
        return (
            f"YannLeCun("
            f"name={self.name}, "
