...
class JoeBiden(CharacterBase):
    """
    Character class for Joe Biden.
    
    This class extends CharacterBase with specific configuration for Joe Biden.
    """
    ...
    def __init__(
        ...
        """
        Initialize Joe Biden character.
        
        Args:
            image_paths: List of paths to Joe Biden images
            voice_samples: List of paths to Joe Biden voice sample audio files
            dataset_path: Path to the Joe Biden dataset folder
            persona_llm_ckpt_path: Path to the Joe Biden persona LLM checkpoint
            video_paths: List of paths to Joe Biden videos
            yaml_path: Path to the joe_biden.yaml config file (defaults to characters/joe_biden/joe_biden.yaml)
        """
        super().__init__(
        ...
        if yaml_path is None:
            # Default to characters/joe_biden/joe_biden.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "joe_biden" / "joe_biden.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "joe_biden" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Joe Biden")
    
    def __repr__(self) -> str:
        """String representation of the Joe Biden character."""
        return (
            f"JoeBiden("
            f"name={self.name}, "
