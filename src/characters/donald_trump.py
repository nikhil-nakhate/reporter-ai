...
class DonaldTrump(CharacterBase):
    """
    Character class for Donald Trump.
    
    This class extends CharacterBase with specific configuration for Donald Trump,
    a former President of the United States known for his direct communication style.
    """
    ...
    def __init__(
        ...
        """
        Initialize Donald Trump character.
        
        Args:
            image_paths: List of paths to Donald Trump images
            voice_samples: List of paths to Donald Trump voice sample audio files
            dataset_path: Path to the Donald Trump dataset folder
            persona_llm_ckpt_path: Path to the Donald Trump persona LLM checkpoint
            video_paths: List of paths to Donald Trump videos
            yaml_path: Path to the donald_trump.yaml config file (defaults to characters/donald_trump/donald_trump.yaml)
        """
        super().__init__(
        ...
        if yaml_path is None:
            # Default to characters/donald_trump/donald_trump.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "donald_trump" / "donald_trump.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "donald_trump" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Donald Trump")
    
    def __repr__(self) -> str:
        """String representation of the Donald Trump character."""
        return (
            f"DonaldTrump("
            f"name={self.name}, "
