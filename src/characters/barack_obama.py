...
class BarackObama(CharacterBase):
    """
    Character class for Barack Obama.
    
    This class extends CharacterBase with specific configuration for Barack Obama,
    a former President of the United States known for his eloquent speaking style.
    """
    ...
    def __init__(
        ...
        """
        Initialize Barack Obama character.
        
        Args:
            image_paths: List of paths to Barack Obama images
            voice_samples: List of paths to Barack Obama voice sample audio files
            dataset_path: Path to the Barack Obama dataset folder
            persona_llm_ckpt_path: Path to the Barack Obama persona LLM checkpoint
            video_paths: List of paths to Barack Obama videos
            yaml_path: Path to the barack_obama.yaml config file (defaults to characters/barack_obama/barack_obama.yaml)
        """
        super().__init__(
        ...
        if yaml_path is None:
            # Default to characters/barack_obama/barack_obama.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "barack_obama" / "barack_obama.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "barack_obama" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Barack Obama")
    
    def __repr__(self) -> str:
        """String representation of the Barack Obama character."""
        return (
            f"BarackObama("
            f"name={self.name}, "
