...
class ElonMusk(CharacterBase):
    """
    Character class for Elon Musk.
    
    This class extends CharacterBase with specific configuration for Elon Musk.
    """
    ...
    def __init__(
        ...
        """
        Initialize Elon Musk character.
        
        Args:
            image_paths: List of paths to Elon Musk images
            voice_samples: List of paths to Elon Musk voice sample audio files
            dataset_path: Path to the Elon Musk dataset folder
            persona_llm_ckpt_path: Path to the Elon Musk persona LLM checkpoint
            video_paths: List of paths to Elon Musk videos
            yaml_path: Path to the elon_musk.yaml config file (defaults to characters/elon_musk/elon_musk.yaml)
        """
        super().__init__(
        ...
        if yaml_path is None:
            # Default to characters/elon_musk/elon_musk.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "elon_musk" / "elon_musk.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "elon_musk" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Elon Musk")
    
    def __repr__(self) -> str:
        """String representation of the Elon Musk character."""
        return (
            f"ElonMusk("
            f"name={self.name}, "
