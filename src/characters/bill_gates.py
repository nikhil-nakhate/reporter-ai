...
class BillGates(CharacterBase):
    """
    Character class for Bill Gates.
    
    This class extends CharacterBase with specific configuration for Bill Gates.
    """
    ...
    def __init__(
        ...
        """
        Initialize Bill Gates character.
        
        Args:
            image_paths: List of paths to Bill Gates images
            voice_samples: List of paths to Bill Gates voice sample audio files
            dataset_path: Path to the Bill Gates dataset folder
            persona_llm_ckpt_path: Path to the Bill Gates persona LLM checkpoint
            video_paths: List of paths to Bill Gates videos
            yaml_path: Path to the bill_gates.yaml config file (defaults to characters/bill_gates/bill_gates.yaml)
        """
        super().__init__(
        ...
        if yaml_path is None:
            # Default to characters/bill_gates/bill_gates.yaml relative to this file
            current_dir = Path(__file__).parent
            yaml_path = current_dir / "bill_gates" / "bill_gates.yaml"
    ...
    def _load_voice_samples(self):
        ...
        current_dir = Path(__file__).parent
        voice_dir = current_dir / "bill_gates" / "voice"
        ...
        if voice_dir.exists():
            ...
            if voice_files:
                ...
                logger.info(f"Auto-loaded {len(self.voice_samples)} voice samples for Bill Gates")
    
    def __repr__(self) -> str:
        """String representation of the Bill Gates character."""
        return (
            f"BillGates("
            f"name={self.name}, "
